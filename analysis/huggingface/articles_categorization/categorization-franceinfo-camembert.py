...
from transformers import EarlyStoppingCallback, CamembertForSequenceClassification, CamembertTokenizer
...
model = CamembertForSequenceClassification.from_pretrained(model_name, num_labels=len(categories_list))

# Define Trainer parameters

dataset_path = "all-articles.json"
model_path = "output/trainer-camembert-small"
...
def preprocess_data():
    ...
    # Preprocess data
    X_train = list(raw_train_ds["article"][:5000])
    y_train = labels.str2int(raw_train_ds["categorie"][:5000])
    X_val = list(raw_validation_ds["article"][:500])
    y_val = labels.str2int(raw_validation_ds["categorie"][:500])
...
def load_model():
    ...
    model = CamembertForSequenceClassification.from_pretrained(model_path, num_labels=len(categories_list))
...
def main():
    ...
    labels = ClassLabel(names=categories_list)
    X_val = list(raw_test_ds["article"][:500])
    y_val = labels.str2int(raw_test_ds["categorie"][:500])
